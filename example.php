<?php
include 'config/mysql.php';//file whith connection
include 'config/funcoes.php';//file whith all the functions of the system
include("funcoes_cef_sigcb.php");//file with all the functions of the billets

//dates to generate the billets
$data= data_brasil_eua($_GET["inicio"]);
$data2=data_brasil_eua($_GET["final"]);

	//consulta banco; check bank
$consulta_b = mysql_query("SELECT * FROM boletos WHERE status='1'") or die (mysql_error());
$n_b 	    = mysql_fetch_array($consulta_b);
$id_banco   = $n_b['id'];
//consulta fatura; check billets
...
$row[]=$n_fatura['id']; //array para loop da gera��o dos boletos;array to loop the generation of billets
}
